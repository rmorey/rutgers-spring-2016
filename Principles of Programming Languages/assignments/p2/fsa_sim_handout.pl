...
state_struct(Name, [state(Name, Transitions, Accepting) | _], state(Name, Transitions, Accepting)).
state_struct(Name, [_ | Rest_states], State_struct) :- 
    state_struct(Name, Rest_states, State_struct).
...
accepts(State, States, Chars) :- 
    next_state_name(State, Chars, Next_name, Rest_chars), 
    state_struct(Next_name, States, Next_state), 
    accepts(Next_state, States, Rest_chars).
